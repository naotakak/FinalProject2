class MapFixer {
    ...
    void trans(double x, double y) {
      ...
      //println("this is the diff of 1 and 0 x cor " + (points[1].getY() - points[0].getY()));
      x = points[3].getX();
      y = points[3].getY();
    ...
    void display() {
        ...
        if (point >= 3){
          //println(headDiff);
          //trans(points[1].getX() , yCor + points[1].getY());
          //trans(points[1].getX(), points[1].getY());
          rotate((float)headDiff);
          points[1].pointAfterRotation((float)(-1 * headDiff));
          //println("point 1 after rotation " + points[1]);
          
          points[3].pointAfterRotation((float)(-1 * headDiff));
          //println("point 3 after rotation " + points[3]);
          //println(" this is Xcxcxoor " + xCor + " this is fhsaklvnyCor " + yCor);
          Point yay = new Point(xCor, yCor);
         
          yay.pointAfterRotation((float)(-1 * headDiff));
           //println("this is new yCor " + yay.getY());
           //println("this is new xCor " + yay.getX());
          trans(points[1].getX()   + (yay.getX() - 505) , yCor + points[1].getY() - yay.getY());
          image(toFix.map, xCor, yCor, (int)toFixWidth, (int)toFixHeight);
          tint(255,255);
        }
        else{
